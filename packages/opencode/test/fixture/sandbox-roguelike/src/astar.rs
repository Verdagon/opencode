...
impl Ord for LGFC {
    fn cmp(&self, other: &Self) -> Ordering {
        match self.f_score.cmp(&other.f_score).reverse() {
            Ordering::Equal => {
                // Break ties by location for determinism
                self.location.cmp(&other.location).reverse()
            }
...
pub fn a_star(
    ...
    if get_adjacent_locations(start_location).contains(&target_location) {
        ...
        if distance <= max_distance {
            return Some((vec![target_location], distance));
        } else {
            return None;
        }
    }
    ...
    while open_locations_lowest_f_first.len() > 0 {
        ...
        if debug_output {
            eprintln!(
                "        [Rust A*] Popped: {:?}, f={}, g={}",
                this_location, this_lgfc.f_score, this_lgfc.g_score
            );
        }
        ...
        for neighbor_loc in neighbors {
            let tentative_g_score = lcfg_by_location[&this_location].g_score
                + get_distance(this_location, neighbor_loc);
            let tentative_f_score = tentative_g_score + get_distance(neighbor_loc, target_location);
            ...
            let new_lgfc = LGFC::new(
                neighbor_loc,
                tentative_g_score,
                tentative_f_score,
                this_location,
            );
            lcfg_by_location.insert(neighbor_loc, new_lgfc);
            if debug_output {
                eprintln!(
                    "        [Rust A*] Pushed: {:?}, f={}, g={}",
                    neighbor_loc, tentative_f_score, tentative_g_score
                );
            }
            open_locations_lowest_f_first.push(new_lgfc);
