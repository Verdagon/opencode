...
pub struct Level {
    ...
    // An index of what entities are at what locations
    pub entity_by_location: FxHashMap<Location, generational_arena::Index>,

    // All entities on this level
    pub entity_indices: Vec<generational_arena::Index>,

    // Squads on this level
    pub squads: Vec<Squad>,

    // Totems on this level
    pub totem_indices: Vec<generational_arena::Index>,
}
...
impl Level {
    pub fn find_random_walkable_unoccuped_location(self: &Level, rand: &mut LCGRand) -> Location {
        ...
        // Filter out locations that are already occupied by entities
        let unoccupied_locations: Vec<Location> = walkable_locations
            .into_iter()
            .filter(|loc| !self.entity_by_location.contains_key(loc))
            .collect();
        let rng_value = rand.next();
        let loc_index = rng_value % (unoccupied_locations.len() as u32);
        return unoccupied_locations[loc_index as usize];
    }
    ...
    pub fn loc_is_walkable(&self, loc: Location, consider_units: bool) -> bool {
        ...
        if consider_units {
            if self.entity_by_location.contains_key(&loc) {
                return false;
            }
        }
        return true;
    ...
    pub fn get_locations_within_sight(
        ...
        while walked_distance_by_loc_to_consider.len() > 0 {
            // Pick the smallest location from the to do list for determinism.
            let this_loc = *walked_distance_by_loc_to_consider
                .keys()
                .min()
                .expect("wat");
            let walked_distance_from_start_to_this = walked_distance_by_loc_to_consider[&this_loc];
        ...
        let mut result: Vec<Location> = visible_locs.into_iter().collect();
        result.sort();
        return result;
    }

    pub fn forget_entity(&mut self, entity_index: generational_arena::Index, loc: Location) {
        assert!(self.entity_by_location.contains_key(&loc));
        assert!(self.entity_by_location[&loc] == entity_index);
        self.entity_by_location.remove(&loc);
    }
    ...
    pub fn get_walkable_locations(&self) -> Vec<Location> {
        ...
        result.sort_by(|a, b| match a.x.cmp(&b.x) {
    ...
    pub fn move_entity(
        ...
        assert!(!self.entity_by_location.contains_key(&new_loc));
        self.entity_by_location.remove(&entity.loc);
        entity.loc = new_loc;
        self.entity_by_location.insert(entity.loc, entity_index);
    }

    // Increment the registration count for the given tile.
    pub fn register_acting_tile(&mut self, loc: Location) {
        let previous_num_registrations = self
            .acting_tile_locations
            .get(&loc)
            .map(|&x| x)
            .or_else(|| Some(0))
            .expect("");
        let new_num_registrations = previous_num_registrations + 1;
        self.acting_tile_locations
            .insert(loc, new_num_registrations);
    }
